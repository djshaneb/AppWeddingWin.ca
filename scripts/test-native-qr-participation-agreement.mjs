...
const notice = '2026-09-04-pre-scan-draw-consent';
...
function fixture(options = {}) {
  const calls = [], accepted = [], writes = [], errors = [], busy = [], backfill = [];
  ...
  const g = {
    ...
    participationProfileEventKey: 'published-event', participationNoticeKey: 'notice.account.event.rules', participationNoticeScope: 'current-context',
    ...
    SecureStore: { getItemAsync: async () => options.cached ? '1' : null,
    ...
    fetchQrBingoJsonWithTimeout: async (_url, request) => {
      ...
      return { response: { ok: options.ok !== false }, data: options.data ?? { ok: true, participation_agreement: receipt } };
    },
  };
  ...
  return { api, g, calls, accepted, writes, errors, busy, backfill, release: () => release?.(),
