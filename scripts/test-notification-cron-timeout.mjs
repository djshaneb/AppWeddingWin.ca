...
try{
 await db.exec(`create schema cron;create schema net;create table cron.job(jobid bigint primary key,jobname text unique,schedule text,active boolean,command text,database text,username text);
 ...
 create function cron.alter_job(job_id bigint,schedule text default null,command text default null,database text default null,username text default null,active boolean default null)returns void language plpgsql as $$begin insert into cron.calls values(job_id);update cron.job j set command=coalesce(alter_job.command,j.command),schedule=coalesce(alter_job.schedule,j.schedule),active=coalesce(alter_job.active,j.active) where j.jobid=job_id;end$$;`);
 ...
 await test('sets 180-second wait and preserves exact headers URL body job identity and schedule',async()=>{
  await db.exec(bound);const after=await row('select to_jsonb(j) value from cron.job j where jobid=2');
 ...
 await test('exact already-applied command is idempotent without another scheduler mutation',async()=>{
  await db.exec(bound);assert.equal((await row('select count(*)::int n from cron.calls')).n,1);
 });
 await test('unexpected command headers body URL timeout or job state fails closed',async()=>{
  ...
  for(const edited of [command.replace('example.test','wrong.test'),command.replace("body := '{}'::jsonb","body := '{\"other\":true}'::jsonb"),applied.replace('180000','300000'),command+' -- changed']){
   await db.query('update cron.job set command=$1 where jobid=2',[edited]);await assert.rejects(db.exec(bound),/Unexpected notification cron command baseline/);assert.equal((await row('select count(*)::int n from cron.calls')).n,1);
  }
  await db.query('update cron.job set command=$1,active=false where jobid=2',[command]);await assert.rejects(db.exec(bound),/Unexpected notification cron job baseline/);
  await db.exec("update cron.job set active=true,schedule='*/2 * * * *' where jobid=2");await assert.rejects(db.exec(bound),/Unexpected notification cron job baseline/);
  await db.exec("update cron.job set schedule='* * * * *',jobid=4 where jobid=2");await assert.rejects(db.exec(bound),/Unexpected notification cron job baseline/);
 });
 await test('missing expected extension signature refuses to mutate',async()=>{
  await db.exec('drop function net.http_post(text,jsonb,jsonb,jsonb,integer)');await assert.rejects(db.exec(bound),/Expected notification scheduler signatures are unavailable/);assert.equal((await row('select count(*)::int n from cron.calls')).n,1);
 });
