...
Updated September 15, 2026 for the build 6 review workflow (`fd088ee`, build configuration `18a0847`); earlier policy and traffic observations retain their stated scope. **Internal submission worksheet; fourteen data-type selections remain saved as an unpublished App Store Connect draft.** This covers native features, embedded WeddingWin/website-builder pages and their service providers. Observed collection is separated from purposes and provider practices that still need confirmation. No Apple privacy selections, purposes or publication state were changed by this documentation update.

The approved notification changes and isolated review backend are implemented and deployed. Production build **1.0.0 (6)** finished, passed archive checks, uploaded and completed Apple processing as Ready to Submit; this is not an App Review submission or physical-device test pass. The controlled API and native simulator review paths were verified; build 6 physical installation and push delivery remain unverified at this checkpoint. Historical build 4 iPad message-alert evidence remains in the [iPad report](IPAD_TESTFLIGHT_BUILD4_REPORT.md), and current build 6 evidence is in [release verification](RELEASE_VERIFICATION_2026-09-15.md#build-6-archive-upload-and-apple-processing). These testing limits do not remove implemented collection from the inventory.
...
### Isolated review records added for build 6
