...
- The current untagged source accepts Apple `@privaterelay.appleid.com` addresses without requiring a personal email, opts actual app-created chat messages into Brilliant Directories transactional reply email while keeping empty thread opens silent, and resolves recipient permissions from the explicit current receive flag, then the active direct-message flag, with the legacy receive field as a final fallback. The shared Deno suite passes **71/71**, including private-relay, chat-email, and chat-membership regressions. The related `bd-complete-profile`, `bd-couple-signup`, `bd-vendor-signup`, and `bd-chat-sync` changes are not yet deployed or proven in TestFlight.
...
- EAS project linkage is configured, but `submit.production.ios.ascAppId`, Apple distribution signing, APNs credentials, the actual auto-incremented build number, and App Store Connect processing remain external.
...
- Before the new relay-safe, transactional chat-email, and membership-fallback behavior is release-ready, deploy and record provenance for `bd-complete-profile`, `bd-couple-signup`, `bd-vendor-signup`, and `bd-chat-sync`; register every outbound account/vendor-contact sender with Apple Private Email Relay; verify SPF/DKIM and bounce handling; and prove required delivery with Hide My Email on physical TestFlight without demanding a personal address.
