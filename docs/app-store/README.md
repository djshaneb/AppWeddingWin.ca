...
Production build `1.0.0 (2)` was uploaded to App Store Connect and Apple marked its binary `Validated`; the version is `Ready to Submit`. It is assigned to the manual `WeddingWin Internal QA` group and the account-holder tester is invited. It predates the current `2026-09-01-in-person-entry` role/rules/vendor-verification/named-vendor contact-sharing-and-marketing delta and is therefore historical evidence, not the current submission candidate. No App Review submission or public release has occurred.
...
- Native private-message photo attachments are restored behind an explicit backend rollout gate. After the user taps Attach, iOS presents its system picker and gives the app only the selected item; the app does not request broad photo-library access for this flow. The app then resizes and re-encodes the chosen asset as a bounded JPEG. The backend checks the declared type, strict base64 and full decode, decoded size, image dimensions, and rollout cutoff before accepting it. This is not automated semantic image moderation; reporting, blocking, conversation closure, and the approved message/media retention rules still apply. Camera use remains limited to QR decoding, and microphone/background-location access is not enabled. Exact TestFlight picker, cancel, attachment, and no-broad-permission-prompt testing on a physical iPhone remains pending.
...
- Focused 2026-08-29 source/backend/Simulator evidence exists for authentication hardening, controlled text chat, account deletion preservation, idempotent QR replay, push reliability logic, and WebView routing. See the matrix; it does not prove the `2026-09-01-in-person-entry` delta and never replaces TestFlight/physical evidence.
- The current source accepts Apple `@privaterelay.appleid.com` addresses without requiring a personal email, opts actual app-created chat messages into Brilliant Directories transactional reply email while keeping empty thread opens silent, and resolves recipient permissions from the explicit current receive flag, then the active direct-message flag, with the legacy receive field as a final fallback. The shared Deno suite passes **195/195**. The related account functions are live as `bd-complete-profile` v24, `bd-couple-signup` v19, and `bd-vendor-signup` v15; chat is live as `bd-chat-sync` v51 and `bd-chat-status` v34 with explicit photo rollout/cutoff controls, bounded per-thread reads, content-free exact unread counts, and a fair delivery batch that keeps paused photos from hiding later text. The earlier v39 authenticated app↔website text round trip remains historical evidence, and a fresh v45 Simulator test delivered captioned and photo-only JPEG messages to Brilliant Directories and retained them after refresh. Apple relay delivery, authenticated website photo visibility, and the full physical system-picker matrix still require TestFlight evidence.

## External release blockers

- EAS build `ed4826b7-3fcb-43c8-93cf-480cd7057739` succeeded as signed App Store IPA `1.0.0 (2)` and passed the recorded artifact checks. Apple processed it as `Validated`, but it predates the `2026-09-01-in-person-entry` delta. Create and validate a replacement signed build from the final immutable source.
