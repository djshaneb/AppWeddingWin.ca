...
- Release-hardening source through commit `83a14c7` is pushed to `origin/codex-fix-native-google-oauth`. The earlier local `v1.0.0-rc.2` tag remains on its original commit and must not be moved. Create and push a new release-candidate tag only after the final signed-build state is committed; no source is yet associated with a signed archive or TestFlight build.
- Current source links `@blair.shane/weddingwin-app` through `extra.eas.projectId`, points the production submit profile to the exact `WeddingWin Canada` App Store record for `ca.weddingwin.app`, and includes required-reason privacy-manifest declarations. EAS now holds the matching distribution certificate, active App Store profile, App Store Connect API key, and sandbox-and-production APNs key. No processed EAS/TestFlight build or App Store distribution archive is recorded yet; merged signed-archive validation and physical iPhone/iPad evidence remain pending.
