...
App Privacy and the remaining device/reviewer/policy gates remain open. A later fresh check corrected the earlier attachment observation: Feedback Assistant draft `120056018` contains the updated build 5/7 report, but **no attachments are uploaded and the report is not submitted**. The prepared local ZIP `work/app-store-build7-sept15/apple-build7-escalation-attachments-private.zip` contains the earlier build 5 error screenshots and the build 7 installation-result JSON. A local ZIP is not an uploaded attachment. No App Review submission or public release is recorded.
...
The latest case-number Gmail search still found only Apple’s acknowledgement for **102964472775**. Feedback Assistant draft `120056018` is saved with the updated build 5/7 report; no attachments are uploaded and it remains unsubmitted. App Privacy and remaining device/policy gates are open. No App Review submission or public release has occurred.
...
Build 7 therefore reproduces the earlier TestFlight distribution symptom despite a verified archive, completed Apple processing and correct internal-group assignment. The evidence does not establish the cause, identify another source/signing defect or justify an additional rebuild. A follow-up for Apple’s existing case **102964472775** is saved in Feedback Assistant draft `120056018`, updated for builds 5 and 7. No attachments are uploaded, and the draft is **not submitted**. The local evidence ZIP is prepared only; it does not establish transmission to Apple. The private Notes are saved, the version draft still selects build 5, and App Privacy and remaining acceptance/policy gates remain open. No App Review submission or public release occurred.
...
This confirms direct installation and native foreground startup on the physical phone. It does **not** verify a fresh login, messages, scans, draw flow, TestFlight delivery or production APNs. The TestFlight build 7 installation failure remains unresolved; neither a distribution root cause nor another necessary code change is established. Apple feedback draft `120056018` remains saved but unsubmitted, with no attachments uploaded. No App Review submission or public release occurred.
