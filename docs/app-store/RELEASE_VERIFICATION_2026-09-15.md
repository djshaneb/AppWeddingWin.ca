...
This is an additional verification record, not a replacement for the historical release matrix or physical-device reports. The checks below passed within their stated scope. App Review submission, final privacy declarations and physical build 5 acceptance remain incomplete.
...
Build 6 is being produced from the approved source with EAS production auto-increment from 5 to 6. Archive validation, TestFlight processing/installation and physical notification acceptance remain separate gates. The previous iPhone installation failure is not presumed fixed by this feature build. Apple Feedback Assistant has the approved report and two original error screenshots attached; its final upload-consent dialog is awaiting the owner's action-time confirmation, so it has not yet been submitted.
