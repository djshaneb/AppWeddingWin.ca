...
- [ ] Complete the coordinated `APP_EMAIL_CHANGE_SECRET` rotation: add the replacement to live Supabase and replace the prior hardcoded secret in Brilliant Directories widget `329` during one explicitly approved sensitive-value update. Never store either value in Git or submission notes.
