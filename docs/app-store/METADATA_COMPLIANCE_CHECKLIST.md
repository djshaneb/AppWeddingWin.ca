...
| Build number | `1` in `app.json` | Current source is explicit. Production EAS has `autoIncrement: true`; record the actual generated build number and confirm App Store Connect has not already used it before upload. |
...
| Unrestricted Web Access | **No only after verification** | WebView host/navigation, bridge-origin, tracking-script, and cookie controls are implemented in source. Against the exact release build, verify arbitrary/off-domain URLs cannot remain browsable in-app, redirects/pop-ups cannot bypass the host policy, and required external links open through the intended system-browser path. Source configuration alone is not evidence. |
...
- [ ] Sign in with Apple capability is enabled on the App ID and distribution profile; the custom Google start/callback flow's production server provider credentials and redirect configuration are approved.
- [ ] Push Notifications capability/APNs key is enabled on the App ID and distribution profile.
- [ ] `extra.eas.projectId` and `submit.production.ios.ascAppId` are configured for the chosen EAS build/submit route. The blank `extra.googleOAuth.iosClientId` is unused by the current custom system-browser flow; remove or document it, then verify the server-held Google credentials and redirect allowlist on TestFlight.
...
- [ ] Content moderation meets Guideline 1.2: filtering, report, app-side member block, timely response and published contact information. Document that an external website entry point may create a fresh thread until sync closes it, or implement and verify website-side prevention before claiming a website-wide user block.
- [ ] Account deletion meets Guideline 5.1.1(v); record disposable-account evidence beyond deactivation under `DEV-10` and obtain owner/legal/product approval for the current behavior that removes the complete shared conversation, including the counterpart's copy.
- [ ] Data collection and deletion match App Privacy answers and the public policy.
- [ ] App review primary/additional credentials and sample QR are prepared.
