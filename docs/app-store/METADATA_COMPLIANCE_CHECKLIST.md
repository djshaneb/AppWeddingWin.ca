...
| --- | --- | --- |
| Messaging and Chat | **Yes** | Users directly exchange private text and, while the explicit backend rollout gate is enabled, user-selected photo attachments. Attachments receive technical file validation, not automated semantic image moderation; report/block and retention controls apply. |
...
- [ ] Private-message photo attachments are disclosed as Photos or Videos and tested on the exact TestFlight build. Verify Attach opens the selected-item iOS system picker without a broad library prompt, cancel/selection, client bounded-JPEG resize/re-encode, backend type/base64/full-decode/decoded-size/dimension/rollout-cutoff rejection, app↔website persistence, report/block, and deletion/retention on a physical iPhone. Do not claim automated semantic image moderation.
