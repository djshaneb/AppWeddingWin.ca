...
- [ ] State exactly which systems are deleted: WeddingWin.ca member/profile and content; Supabase auth/profile and Apple mapping; chat threads/messages/retained media/outbox where legally and technically permitted; push tokens; QR scan data; raffle entries/draws; backups on expiry.
- [ ] Decide and disclose the effect on the other conversation participant. The deployed purge currently removes the complete shared conversation, including the counterpart's copy. Make the confirmation warning, policy, retention schedule, and implementation agree after owner/legal/product review.
