...
async function harness(url: URL, mode: typeof modes[number]) {
  ...
  const dependencies = {
    ...
    vendorPrizeEditStatus: async () => ({
      prize_editable: !currentRows().some(row => row.couple_email_sent_at),
      prize_details_locked: currentRows().some(row => row.couple_email_sent_at),
      prize_details_lock_reason: currentRows().some(row => row.couple_email_sent_at) ? "sent" : null,
      prize_edit_deadline_at: "2026-10-18T15:00:00Z",
