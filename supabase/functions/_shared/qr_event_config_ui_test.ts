...
Deno.test(
  'native QR scanner renders and refreshes canonical event configuration',
  async () => {
    ...
    assert(
      includesIgnoringWhitespace(
        source,
        'function normalizeQrBingoEventConfig(value: unknown)',
      ) &&
    ...
    assert(
      source.includes('const clearBingoCardState = useCallback(() => {') &&
        ...
        /if \(!nativeSession\?\.user_id \|\| !nativeSession\?\.token\) \{[\s\S]*?clearBingoCardState\(\);/.test(
          source,
        ) &&
        (
          source.match(
        ...
        /catch \(error\) \{\s*if \(requestId !== bingoCardRequestIdRef\.current\) return;\s*clearBingoCardState\(\);/.test(
          source,
        ),
    ...
    assert(
      source.includes('venue_name: string;') &&
        source.includes('app_card_enabled: boolean;') &&
        source.includes("typeof payload.app_card_enabled !== 'boolean'") &&
        source.includes("typeof payload.scan_enabled !== 'boolean'") &&
        source.includes('normalizedText(payload.venue_name, 160)') &&
        source.includes('QR_BINGO_PUBLIC_CONFIG_URL') &&
        source.includes('QR_BINGO_MENU_CONFIG_REFRESH_MS = 60_000') &&
        source.includes('refreshQrMenuEventConfig') &&
        source.includes("if (state === 'active')") &&
        source.includes("AppState.currentState === 'active'") &&
        source.includes('clearInterval(refreshTimer)') &&
        source.includes('formatQrMenuDate') &&
        source.includes('timeZone: PROMOTION_TIME_ZONE') &&
        source.includes('Available at Wedding Shows') &&
        source.includes('Wedding show details are unavailable') &&
        source.includes("qrMenuEventConfig?.event_name || 'Wedding Show'") &&
        source.includes('{qrMenuEventNameLabel}') &&
        source.includes(
          '`Open ${qrMenuEventNameLabel} QR Bingo scanner. ${qrMenuScheduleLabel}`',
        ) &&
        source.includes('setQrMenuConfigUnavailable(true)') &&
        source.includes('if (!eventConfig)') &&
        !source.includes("'Next wedding show'") &&
        source.includes('disabled={!qrMenuAvailable}') &&
        includesIgnoringWhitespace(
          source,
        ...
        includesIgnoringWhitespace(
          source,
          'accessibilityState={{ disabled: !qrMenuAvailable, busy: qrMenuConfigPending }}',
        ) &&
        !source.includes(
          'Booth scanning opens at the Niagara Wedding Show on October 18',
        ),
      'the couple home card must name the configured wedding show, load synchronized show details, fail closed when unavailable, expose an accessible disabled state, and avoid hardcoded dates',
    );
  },
);

Deno.test(
  'native QR network calls time out and preserve backend raffle errors',
  async () => {
    const source = await Deno.readTextFile(
      new URL('../../../app/(tabs)/index.tsx', import.meta.url),
    );
...
Deno.test('vendor draw close flushes dirty autosave state', async () => {
  ...
  assert(
    ...
    'closing dirty vendor settings must serialize pending autosaves, await a successful current-generation save, and remain open on failure or a newer edit',
  );
  assert(
    includesIgnoringWhitespace(
      source,
...
Deno.test(
  'vendor draw requests time out without bypassing existing response handling',
  async () => {
    const source = await Deno.readTextFile(
      new URL('../../../app/(tabs)/index.tsx', import.meta.url),
    );
    ...
    assert(
      getAction >= 0 &&
        ...
        entriesGetAction > updateAction &&
        entryUpdateAction > entriesGetAction &&
        drawAction > entryUpdateAction &&
        sendNoticeAction > drawAction &&
        (
          source.match(
  ...
  async () => {
    ...
    assert(
      source.includes('const drawVendorWinner = async () => {') &&
        ...
        source.includes('const actionKey = `send-winner:${draw.id}`;') &&
        source.includes(
...
Deno.test(
  'development QR emulation is restricted to isolated fixtures',
  async () => {
    const source = await Deno.readTextFile(
      new URL('../../../app/(tabs)/index.tsx', import.meta.url),
    );
