...
Deno.test("in-person cutover preserves historical rows and blocks only new alternate entries", async () => {
  const sql = await Deno.readTextFile(migrationUrl);
  const guard = sqlFunction(
    sql,
    "reject_new_qr_bingo_alternate_free_entry",
  );
...
Deno.test("every new QR draw entry requires paired server scan proof", async () => {
  const sql = await Deno.readTextFile(migrationUrl);
  ...
  for (const endpointUrl of endpointUrls) {
    ...
    const optInCall = source.indexOf(
      "await optInToRaffle(",
      actionStart,
    );
...
Deno.test("Form 354 reconciliation is tombstoned without deleting its audit history", async () => {
  const sql = await Deno.readTextFile(migrationUrl);
...
Deno.test("winner selection keeps its exact-vendor lock but no longer waits for Form 354 closure", async () => {
  const sql = await Deno.readTextFile(migrationUrl);
...
Deno.test("in-person rules transition changes only the version and published show opening", async () => {
...
Deno.test("current database consent and proof use the published in-person window", async () => {
  const sql = await Deno.readTextFile(rulesMigrationUrl);
  const rules = sqlFunction(sql, "enforce_qr_bingo_current_rules");
  const proof = sqlFunction(
    sql,
    "require_new_qr_bingo_in_show_scan_proof",
  );
...
Deno.test("in-person config publication is locked, idempotent, and fails closed", async () => {
  ...
  for (
    const required of [
      ...
      "current_config.rules_version is distinct from previous_rules_version",
    ...
    assert(sql.includes(required), `config publication is missing ${required}`);
  }
  ...
  assert(
    lockAt >= 0 && currentAt > lockAt && noOpAt > currentAt &&
      writeAt > noOpAt,
...
Deno.test("in-person config publication clones every other field and writes normal audit history", async () => {
  const sql = await Deno.readTextFile(publishMigrationUrl);
