-- Vendor draws are now entered only after a saved in-show booth QR scan.
-- Historical alternate-entry audit rows and immutable offer snapshots remain
-- intact; this migration blocks only new alternate entries and removes the
-- now-obsolete Form 354 reconciliation gates from future draws.
...
alter table public.qr_bingo_raffle_entries
  drop constraint if exists qr_bingo_raffle_entries_in_show_scan_proof_complete;
alter table public.qr_bingo_raffle_entries
  add constraint qr_bingo_raffle_entries_in_show_scan_proof_complete
  check (
    (
      in_show_scan_verified is null
      and in_show_scan_verified_at is null
    )
    or (
      in_show_scan_verified is true
      and in_show_scan_verified_at is not null
    )
  ) not valid;
alter table public.qr_bingo_raffle_entries
  validate constraint qr_bingo_raffle_entries_in_show_scan_proof_complete;

comment on column public.qr_bingo_raffle_entries.in_show_scan_verified is
  'Server attestation that this QR draw opt-in followed a saved scan of the exact vendor booth. NULL identifies a historical row created before this proof was introduced.';
comment on column public.qr_bingo_raffle_entries.in_show_scan_verified_at is
  'Server timestamp paired with in_show_scan_verified. NULL is retained for historical rows.';
...
revoke all on function public.reject_new_qr_bingo_alternate_free_entry()
  from public, anon, authenticated, service_role;

drop trigger if exists a00_reject_new_qr_bingo_alternate_entry
...
on public.qr_bingo_raffle_entries
for each row execute function public.reject_new_qr_bingo_alternate_free_entry();

comment on function public.reject_new_qr_bingo_alternate_free_entry() is
  'Forward-only guard: rejects new or changed-to alternate-free-entry rows while allowing unchanged historical rows to remain auditable.';
...
as $$
begin
  if new.entry_method = 'qr_scan_opt_in'
    and (
...
drop trigger if exists a01_require_new_qr_bingo_in_show_scan_proof
  on public.qr_bingo_raffle_entries;
create trigger a01_require_new_qr_bingo_in_show_scan_proof
before insert on public.qr_bingo_raffle_entries
for each row execute function public.require_new_qr_bingo_in_show_scan_proof();

comment on function public.require_new_qr_bingo_in_show_scan_proof() is
  'Requires every new QR draw entry to carry the server-stamped proof produced only after the exact vendor booth scan check.';

-- Keep the old service signature callable during a rolling deploy, but make it
-- an explicit tombstone so stale Form 354 admin code cannot create an entry.
...
begin
  ...
  return jsonb_build_object(
    ...
    'error', 'Vendor draw entry is available only after scanning that vendor booth QR code at the wedding show.'
  );
end;
$$;
...
comment on function public.reconcile_qr_bingo_alternate_free_entry(
  ...
  'Disabled compatibility tombstone. Historical Form 354 rows remain intact, but no new alternate-free-entry row may be reconciled.';

-- Form 354 reconciliation is no longer a prerequisite for a booth-scan draw.
-- Keep its historical trigger function and closure ledger for audit evidence.
