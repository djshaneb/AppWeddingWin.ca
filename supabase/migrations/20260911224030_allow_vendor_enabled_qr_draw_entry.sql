-- Require the exact reviewed production lifecycle baseline before changing it.
do $guard$ begin
 if (select md5(pg_get_functiondef(p.oid)) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='enforce_qr_bingo_current_rules') is distinct from '78053d54dac16c545980cafe71f2658b' then raise exception 'Unexpected baseline for enforce_qr_bingo_current_rules'; end if;
 if (select md5(pg_get_functiondef(p.oid)) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='enforce_qr_bingo_entry_offer_version') is distinct from '8c73794b581882ab33d1948c09d295be' then raise exception 'Unexpected baseline for enforce_qr_bingo_entry_offer_version'; end if;
 if (select md5(pg_get_functiondef(p.oid)) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='require_new_qr_bingo_in_show_scan_proof') is distinct from 'c7bcae1279ad83faba5208ecb1e2726f' then raise exception 'Unexpected baseline for require_new_qr_bingo_in_show_scan_proof'; end if;
 if (select md5(pg_get_functiondef(p.oid)) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='select_qr_bingo_potential_winner') is distinct from '05f2e14a2c3a5c8e2b9964cc4b0d349f' then raise exception 'Unexpected baseline for select_qr_bingo_potential_winner'; end if;
end; $guard$;
-- Organizer-authorized QR entry access. Never rewrites old entries, vendor
-- acceptance, immutable offers, show dates, draw opening or closing times.
alter table public.qr_bingo_raffle_entries
 add column vendor_draw_scan_verified boolean,
 add column vendor_draw_scan_verified_at timestamptz,
 add column vendor_draw_scan_config_revision bigint,
 add column entry_access_policy_version text,
 add column entry_access_policy_disclosure text,
 add column entry_access_applied_at timestamptz;

create or replace function public.qr_bingo_entry_access_policy_disclosure()
...
create or replace function public.qr_bingo_effective_entry_disclosure(original text)
returns text language sql immutable security invoker set search_path='' as $$
 select case when array_length(string_to_array(original,'By entering, I confirm that I visited this vendor booth in person at the wedding show and scanned its QR code.'),1)=2
 then replace(original,'By entering, I confirm that I visited this vendor booth in person at the wedding show and scanned its QR code.','By entering, I confirm that I scanned this vendor’s QR code during authorized QR Bingo scanning.') else null end;
$$;
create or replace function public.qr_bingo_entry_access_evidence_valid(entry public.qr_bingo_raffle_entries)
...
$$;
alter table public.qr_bingo_raffle_entries add constraint qr_bingo_entry_access_evidence_coherent check(
 (vendor_draw_scan_verified is null and vendor_draw_scan_verified_at is null and vendor_draw_scan_config_revision is null
  and entry_access_policy_version is null and entry_access_policy_disclosure is null and entry_access_applied_at is null)
 or public.qr_bingo_entry_access_evidence_valid(qr_bingo_raffle_entries.*));
comment on column public.qr_bingo_raffle_entries.vendor_draw_scan_verified is
 'Server-authorized QR scan proof for current scanner access plus vendor opt-in. Early proof does not claim an in-show physical visit. Historical in_show_scan evidence remains unchanged.';
comment on column public.qr_bingo_raffle_entries.entry_access_policy_disclosure is
 'Exact server-applied entry-access policy at the time of the couple’s entry choice. This is configuration evidence, not a separate acceptance. Original immutable vendor offer and vendor acceptance are preserved.';
revoke all on function public.qr_bingo_entry_access_policy_disclosure(),public.qr_bingo_effective_entry_disclosure(text),public.qr_bingo_entry_access_evidence_valid(public.qr_bingo_raffle_entries) from public,anon,authenticated;
grant execute on function public.qr_bingo_entry_access_policy_disclosure(),public.qr_bingo_effective_entry_disclosure(text),public.qr_bingo_entry_access_evidence_valid(public.qr_bingo_raffle_entries) to service_role;
...
$function$;
drop trigger if exists a01_require_new_qr_bingo_in_show_scan_proof on public.qr_bingo_raffle_entries;
create trigger a01_require_new_qr_bingo_in_show_scan_proof before insert or update of
 vendor_offer_version,consent_version,promotion_disclosure_text,event_key,vendor_bingo_id,vendor_bd_user_id,couple_bd_user_id,
 in_show_scan_verified,in_show_scan_verified_at,vendor_draw_scan_verified,vendor_draw_scan_verified_at,vendor_draw_scan_config_revision,
 entry_access_policy_version,entry_access_policy_disclosure,entry_access_applied_at
 on public.qr_bingo_raffle_entries for each row execute function public.require_new_qr_bingo_in_show_scan_proof();
...
comment on column public.qr_bingo_raffle_entries.in_show_scan_verified is
 'Historical or current in-show proof only. NULL is valid for an authorized early QR entry with separate vendor_draw_scan_verified evidence; early entry never claims show attendance.';
